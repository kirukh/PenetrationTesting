...
> **🎯 Prüfung:** Man wird **nicht immer voll root**. Oft reicht es, per SUID-Binary eine
> Shell mit **euid=root** (`-p`) zu bekommen oder eine Datei mit Root-Rechten zu lesen — das
> genügt schon, um `root.txt` zu lesen. Ziel ist die **Flag**, nicht zwingend eine Root-Shell.
> ```bash
> ./<suid-binary> -p        # bzw. der passende GTFOBins-Befehl -> euid root
> cat /root/root.txt        # Flag lesen reicht
> ```
