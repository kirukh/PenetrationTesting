...
>
> **➡️ Prüfungsrelevant:** Genau dieses Muster (ein Dienst, den ein normaler User
> ändern/neu starten darf, läuft aber als SYSTEM) ist ein erwartbarer Windows-PrivEsc-Weg.
> Erst mit `sc qc <dienst>` prüfen, **als wer** der Dienst läuft, dann mit `PrivescCheck`
> die Änderungsrechte bestätigen. Statt einer Reverse-Shell genügt oft ein simpler
> `cmd.exe /c net localgroup Administrators <user> /add` als binPath.
...
# Rekursiv nach Keywords in abgelegten Dateien (Notizen, Configs, Backups)
powershell -c "Get-ChildItem C:\Users -Recurse -Force -Include *.txt,*.xml,*.config,*.ini,*.cfg,*.log,*.kdbx -EA SilentlyContinue | Select-String -Pattern 'password|passwort|secret|cred|admin' -EA SilentlyContinue"
...
> **Lab-Beispiel (Stored Creds):** Auf dem System lag in einer abgelegten Notiz-Datei
> ein Klartext-Passwort. Solche Dateien auf Desktop/Dokumente **immer** prüfen — sie sind
> oft der Schlüssel zum nächsten Schritt (Login als anderer User, Zugriff auf einen Dienst).
> **Prüfungs-Hinweis:** „Eine Userin hat eine Anwendung benutzt und dort Hinweise hinterlegt"
> — also auch App-Verzeichnisse, History und Backups durchsehen.
...
- [Windows LPE Cookbook](https://github.com/nickvourd/Windows-Local-Privilege-Escalation-Cookbook) — vom Prof empfohlen
