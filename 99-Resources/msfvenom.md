...
> **⚠️ Prüfung — staged vs. stageless (Listener muss passen!):** Der Listener muss **denselben**
> Payload erwarten, mit dem du generiert hast.
> - **stageless** (`shell_reverse_tcp`, `meterpreter_reverse_tcp` — mit Unterstrich): kompletter
>   Payload im File. Geht mit `nc -lvnp 4444` **oder** `multi/handler` + selbem stageless-Payload.
> - **staged** (`shell/reverse_tcp`, `meterpreter/reverse_tcp` — mit Slash): lädt den 2nd-Stage
...
> generierten Payload-String, um später den richtigen Handler zu setzen.
